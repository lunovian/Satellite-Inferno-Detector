...
class SatelliteFireAugmentation:
    ...
    def augment_dataset(self, dataset_dir, output_dir, augmentation_factor=2):
        ...
        """
        if has_rich_console:
            print_section("Dataset Augmentation")
            print_info(f"Source directory: {dataset_dir}")
        ...
        if os.path.exists(yaml_src):
            ...
            # Get absolute paths for new data locations
            train_dir = os.path.join(os.path.abspath(output_dir), "train", "images")
            val_dir = os.path.join(os.path.abspath(output_dir), "valid", "images")
            test_dir = os.path.join(os.path.abspath(output_dir), "test", "images")

            # Update paths directly with absolute paths
            yaml_data["train"] = train_dir
            yaml_data["val"] = val_dir
            if "test" in yaml_data and yaml_data["test"]:
            ...
            if "names" not in yaml_data:
                yaml_data["names"] = ["Fire"]

            if "task" not in yaml_data:
                yaml_data["task"] = "detect"
            ...
            if has_rich_console:
                print_success("Created data.yaml with absolute paths:")
                print_info(f"  Train: {train_dir}")
                print_info(f"  Val: {val_dir}")
                print_info(
                    f"  Test: {test_dir if 'test' in yaml_data and yaml_data['test'] else 'Not specified'}"
                )
            else:
                print(
                    f"Updated data.yaml with absolute paths:\n  train: {train_dir}\n  val: {val_dir}"
                )
        ...
        for split in ["train", "valid", "test"]:
            ...
            if not os.path.exists(src_img_dir):
                if has_rich_console:
                    print_warning(f"Directory not found, skipping: {src_img_dir}")
                else:
                    print(f"Directory not found, skipping: {src_img_dir}")
                continue
            ...
            if has_rich_console:
                print_section(f"Processing {split} split")
                print_info(f"Copying original {split} files...")
            else:
                print(f"Copying original {split} files...")

            img_files = glob.glob(os.path.join(src_img_dir, "*"))

            progress = create_progress_bar(f"Copying {split} files")
            if progress:
                task = progress.add_task(
                    f"Copying {len(img_files)} files", total=len(img_files)
                )
                progress.start()
    ...
    def _augment_split(
        ...
        progress = create_progress_bar("Augmenting images")
        if progress:
            task = progress.add_task("Generating augmentations", total=len(img_files))
            progress.start()
        ...
        for idx, img_file in enumerate(img_files):
            ...
            if image is None:
                if has_rich_console:
                    print_warning(f"Could not read {img_file}, skipping.")
                else:
            ...
            for i in range(augmentation_factor):
                try:
                    ...
                    aug_img_name = (
                        f"{base_name}_aug{i + 1}{os.path.splitext(img_name)[1]}"
                    )
                    ...
                    if has_rich_console:
                        print_error(f"Error augmenting {img_name}: {str(e)}")
                    else:
...
def create_augmented_dataset(
    ...
    if has_rich_console:
        ...
        print("\n===== Satellite Fire Image Augmentation =====")
    ...
    if has_rich_console:
        print_success(
            f"Augmentation complete. Augmented dataset created at {output_dir}"
        )
    else:
        print(f"Augmentation complete. Augmented dataset created at {output_dir}")
